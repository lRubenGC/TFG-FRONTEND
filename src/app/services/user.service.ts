...
export class UserService {
  ...
  async getUserData(id: number): Promise<userInterfaceApi | any> {
    const response = await this.http.get(`https://api.diecasttracker.com/api/users/${id}`).toPromise();
    return response as userInterfaceApi;
  }

  async getUserByUsername(username: string): Promise<userInterfaceApi | any> {
    const response = await this.http.get(`https://api.diecasttracker.com/api/users/username/${username}`).toPromise();
    return response as userInterfaceApi;
  }
  ...
  updateUser(id_user: number, userParams: userUpdateRequest): Observable<any> {
    ...
    return this.http.put(`https://api.diecasttracker.com/api/users/${id_user}`, requestBody, { headers });
  }
  ...
  updateImg(id_user: number, file: File, bg: boolean = false): Observable<any> {
    ...
    if (bg) {
      return this.http.post(`https://api.diecasttracker.com/api/users/bg-img/${id_user}`, formData, { headers });
    }
    
    return this.http.post(`https://api.diecasttracker.com/api/users/img/${id_user}`, formData, { headers });
  }
  ...
  deleteImg(id_user: number, bg: boolean = false): Observable<any> {
    ...
    if (bg) {
      return this.http.delete(`https://api.diecasttracker.com/api/users/bg-img/${id_user}`, { headers });
    }
    
    return this.http.delete(`https://api.diecasttracker.com/api/users/img/${id_user}`, { headers });
  }
