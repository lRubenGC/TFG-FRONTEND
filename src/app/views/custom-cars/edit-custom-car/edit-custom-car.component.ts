...
export class EditCustomCarComponent implements OnInit {
  ...
  async submitForm() {
    ...
    if (
      Object.keys(bodyRequest).length === 0 &&
      this.images[0] === null &&
      this.images[1] === null &&
      this.images[2] === null &&
      this.images[3] === null
    ...
    try {
      ...
      if (Object.keys(bodyRequest).length > 0) {
        await this.customCarsService
          .updateCustomCar(this.userToken.userId!, this.carData.id, bodyRequest)
          .toPromise();
      ...
      if (this.userToken.hasToken && this.userToken.userId) {
        for (let i = 0; i < this.images.length; i++) {
          ...
          if (file instanceof File) {
            imgUploaded = true;
            await this.customCarsService
              .uploadImg(this.userToken.userId, this.carData.id, file)
              .toPromise();
          }
        }
  ...
  deleteImg(event: any, type: number) {
    ...
    if (
      event.explicitOriginalTarget.className !== 'button-delete-config' &&
      event.explicitOriginalTarget.className !== 'fas fa-trash'
    ) {
      return;
    }
  ...
  deleteImgValidations(type: number): boolean {
    if (
      type === 0 &&
      (this.carData.imgs[0] === '' || this.carData.imgs[0] === undefined)
    ) {
      ...
      type === 1 &&
      (this.carData.imgs[1] === '' || this.carData.imgs[1] === undefined)
    ) {
      this.errorMsg = 'CANNOT_DELETE_NO_IMG';
      this.formError = true;
      return false;
      ...
      type === 2 &&
      (this.carData.imgs[2] === '' || this.carData.imgs[2] === undefined)
    ) {
      this.errorMsg = 'CANNOT_DELETE_NO_IMG';
      this.formError = true;
      return false;
      ...
      type === 3 &&
      (this.carData.imgs[3] === '' || this.carData.imgs[3] === undefined)
    ) {
      this.errorMsg = 'CANNOT_DELETE_NO_IMG';
      this.formError = true;
      return false;
  ...
  async deleteImgApi(index: number) {
    try {
      ...
      await this.customCarsService
        .deleteImg(this.userToken.userId!, this.carData.id, img)
        .toPromise();
