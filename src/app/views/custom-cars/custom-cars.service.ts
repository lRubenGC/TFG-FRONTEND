...
export class CustomCarsService {
  ...
  getCustomCars(userCreator?: number): Observable<any> {
    if (!userCreator) {
      return this.http.get(`:8000/api/custom-cars`);
    }
    return this.http.get(`:8000/api/custom-cars?userCreator=${userCreator}`);
  }

  getCarById(carId: number): Observable<any> {
    return this.http.get(`:8000/api/custom-cars/${carId}`);
  }
  ...
  uploadCustomCar(id_user: number, carBody: any): Observable<any> {
    ...
    return this.http.post(`:8000/api/custom-cars/${id_user}`, {
  ...
  updateCustomCar(id_user: number, id_car: number, userParams: customCarUpdateRequest): Observable<any> {
    ...
    return this.http.put(`:8000/api/custom-cars/update/${id_user}/${id_car}`, requestBody, { headers });
  }
  ...
  uploadImg(id_user: number, id_car: number, file: File): Observable<any> {
    ...
    return this.http.post(`:8000/api/custom-cars/upload/${id_user}/${id_car}`, formData, { headers });
  }
  ...
  deleteImg(id_user: number, id_car: number, id_img: string): Observable<any> {
    ...
    return this.http.post(`:8000/api/custom-cars/delete/${id_user}/${id_car}`, {
      imgId: id_img
    }, { headers });
  }
